...
\end{equation}
where $A_r$~[\si{ha}] is the land area available for afforestation in node~$r$, derived from the CORINE Land Cover (CLC) dataset as described in \citet{Fernandes2026}, and $\overline{\text{MAI}}_{\text{CO}_2,\,r}$ [\si{tCO_2\,ha^{-1}\,yr^{-1}}]
is the node-level CO$_2$ sequestration rate. The land estimation from CLC is presented in detail in \citet{Fernandes2026} and is identical for both methods described in this document. This section focuses exclusively on the derivation of the CO$_2$ sequestration rates and their temporal disaggregation, and presents a comparison between the updated growth-based method (introduced here) and the original density-based method of \citet{Fernandes2026}. Both methods are available in PyPSA-Eur, selectable via the
\texttt{potential\_type} key in the \texttt{afforestation} configuration block.

% =============================================================================
\subsubsection{Key Aspects of this methodology}
\label{sec:key_aspects}

\paragraph{Key aspects.}
The method focuses on the estimation of the Mean Annual Increment over the optimal rotation age for a new forest, the optimal rotation age and the forest density (in tCO2 eq.) of the forest at the optimal rotation age.
The underlying assumption for afforestation is that beyond the optimal rotation age the forest carbon stock must be maintained at least at that density level for the future years, which is in line with EU legislation.

\begin{itemize}[leftmargin=2em]

    \item \textbf{Afforestation potential}.The Afforestation potential per node  (Eq.~\ref{eq:node_potential}) is obtained from Mean Annual Increment during optimal rotaiton age of a new forest.
    \item \textbf{Compliance with EU legislation}. Beyond the optimal rotation age the forest stock is kept constant, assuming a margin for disturbances and minor harvest. The efficiency is assumed to be 80%.
    \item \textbf{Cost for afforestation}. The cost for afforestation is obtained from literature (EUR/ha). The cost is annualized using the optimal rotation age rather than a fixed lifetime, reflectign the impact of different growth rates on the investment.
    \item \item \textbf{Net Annual Increment} (NAI, m$^3$\,ha$^{-1}$\,yr$^{-1}$):
    ...
    \item \textbf{Biomass Conversion and Expansion Factors} (BCEF):
          age-class-resolved factors [t$_{\text{DM}}$\,m$^{-3}$]
          converting merchantable volume to total aboveground dry
          biomass, derived from species-specific allometric equations
          \citep{Boudewyn2007}.
\end{itemize}
used in pypsa-eur and the optimal rotation age can be used for discounting of the investment in afforestation, accounting for the different forest growth rates in Europe.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Biomass Conversion and Expansion Factors} (BCEF):
          ...
          derived from species-specific allometric equations
          \citep{Boudewyn2007}.
\end{itemize}
...
Figure~\ref{fig:mai_crcf} on the left illustrates graphical definition of optimal rotation age.
...
\begin{quote}
\small
{\footnotesize\texttt{GPP.RS\_METEO.FP-ALL.MLM-ALL.METEO-ERA5.720\_360.daily.\{year\}.nc}}\\[2pt]
...
\texttt{ftp.bgc\allowbreak-jena.mpg.de} (Max Planck Institute for Biogeochemistry,
Jena, Germany).
\end{quote}
...
solve time without increasing the level of details in the results.
...
Before temporal plotting, \texttt{afforestation\_postprocess.py}
redistributes link flows and store state of charge per the GPP weights.
It is called automatically from \texttt{plot\_balance\_\allowbreak timeseries.py}
and \texttt{plot\_heatmap\_\allowbreak timeseries.py}; all energy-system
quantities are unchanged.

% =============================================================================
\subsubsection{Implementation of cost based on optimal rotation age}
\label{sec:cost }
...
Two annualisation strategies are available, selected via \texttt{use\_discount\_rate} in the \texttt{afforestation} configuration block:

\begin{description}
  \item[No discounting (\texttt{use\_discount\_rate: false}).]
  The investment $I$~[\si{EUR\,ha^{-1}}] is spread uniformly over the rotation
  and expressed per unit of CO$_2$:
  \[
  ...
  A node-specific annuity factor $\mathrm{a}(r_d,\,T^*_r)$ is applied, where $r_d$ is the project discount rate (config key \texttt{discount\_rate}):
  \[
...
capital costs at NUTS-2 level under both methods, computed without discounting ($r=0$).
In the growth method, the cost varies substantially across NUTS-2 regions because it depends on the
local rotation age $T^*_r$ and the region-specific MAI: boreal regions (Scandinavia, Finland)
and slow-rotation forests (Portugal) carry higher unit costs, while productive temperate forests
in central Europe (France, Germany) have lower costs.
In the density method, costs vary only at country level because the underlying Avitabile biomass
density is a national-level statistic; spatial variation within a country is absent.
Both methods yield area-weighted average costs of similar magnitude (140--190~\si{EUR\,tCO_2^{-1}}),
confirming that the choice of formula matters less than the local forest growth characteristics.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/fig_affo_pypsa_comparison_capex.png}
    \caption{Capital cost of afforestation CDR at NUTS-2 level [\si{EUR\,tCO_2^{-1}}],
    computed without discounting ($r=0$) using 2030 technology costs
    ($I=9743$~\si{EUR\,ha^{-1}}, $\mathrm{FOM}=5\,\%\,\mathrm{yr}^{-1}$, $L=30\,\mathrm{yr}$).
    (a)~Growth method: $I\cdot(1/T^*_r+\mathrm{FOM})/\overline{\mathrm{MAI}}_{\mathrm{CO}_2,\,r}$,
    using the region-specific rotation age $T^*_r$ from the Pilli yield-table data.
    (b)~Density method: $(I+\mathrm{FOM}\cdot L\cdot I)/(\rho_{\mathrm{wood}}\cdot c)$,
    where $\rho_{\mathrm{wood}}$ is the national-level biomass density from \citet{Avitabile2024}
    and $c=1.83$~\si{tCO_2\,t_{DM}^{-1}}.
    Grey regions lack biomass data and use a European fallback of 117~\si{t_{DM}\,ha^{-1}}.
    Area-weighted averages are shown in the panel footers.}
...
The method presented here has some limitations. The method does not allow for the introduction of species and forest managemt types if not already present in a region. This limit can lead both to an under estimation of the potential (preventing the introduction of extint but productive species in an region) or over estimation due to the application of the NUTS2 propagation mechanism. Biodiversity is considered only using a conservative assumption on the conifer/broadleaf blending. In general the two main sources of uncertanity on the afforestation potential are lack of data in some European regions and the estimation of the Land availale for afforestation from CLC.
...
2.1 to 12.6~\si{tCO_2\,ha^{-1}\,yr^{-1}}
across 114~NUTS-2 regions with
direct Pilli data, with a European median of 5.2~\si{tCO_2\,ha^{-1}\,yr^{-1}}
and mean of 5.5~\si{tCO_2\,ha^{-1}\,yr^{-1}}. These values are consistent with
the 5--15~\si{tCO_2\,ha^{-1}\,yr^{-1}} range reported for temperate European
...
\end{tabular}
\par\vspace{4pt}
{\footnotesize \textsuperscript{a}The growth method does not apply a
separate $f_{\text{land}}$ scaling; the land-utilisation assumption
is embedded in the rotation-averaged MAI calculation.}
\end{table}

\paragraph{Discussion: capital cost calculation.}
The two methods compute the afforestation capital cost differently,
and the density-method formula contains a dimensional inconsistency.
The relevant code in
\texttt{prepare\_sector\_network.py} is reproduced
below for comparison (original density-mode implementation)\footnote{%
  \href{https://github.com/ricnogfer/pypsa-eur/blob/c1e990cd634cf236e0fbcedcb46570057177ed6f/scripts/prepare_sector_network.py\#L1037--L1048}{%
    ricnogfer/pypsa-eur, \texttt{prepare\_sector\_network.py}, lines 1037--1048}.}.
...
capital_cost = (
    (investment_cost + maintenance_cost)
    / (densities * co2_per_tonne)
)
# Units: (EUR/ha + EUR*yr/ha) / (t_DM/ha * t_CO2/t_DM)
#       = EUR/tCO2 * (1 + yr)  -- dimensionally inconsistent!
\end{verbatim}
